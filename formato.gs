...
function resetSheet(sheet) {
  ...
  // Paso 1: quitar formato Texto (tipo @) heredado del .xlsx
  // Si falla el rango completo (chips / columnas texto), se hace columna a columna
  try {
    ...
    for (let c = 1; c <= lc; c++) {
      try {
        ...
        // Columna incompatible (chips, validaciones de texto) → se omite sin detener
      }
    }
  }

  // Paso 2: reset visual completo, cada propiedad protegida
...
function applyFmt(cell, label) {
  try {
    ...
    if (hasAny(l, ['baja', 'beneficio', 'gastos generales', 'iva', '%', 'peso criterio', 'porcentaje', 'margen'])) {
      fmt = '0.00%';
    } else if (hasAny(l, ['pm', 'personas adscritas', 'precio hora', 'hora real', 'experiencia', 'coste empresa + experiencia'])) {
      fmt = '#,##0.00';
    } else if (hasAny(l, ['horas', 'dias', 'días', 'duracion', 'duración', 'meses', 'licitadores', 'licitador', 'ofertas', 'lotes', 'mes_inicio', 'mes_final'])) {
      fmt = '#,##0';
    }
...
function formatGantt(sheet) {
  ...
  for (let r = 1; r < firstData; r++) {
    ...
    if (isYear)       { bg = C.DARK;     h = 26; fs = 10; }
    else if (isMeta)  { bg = C.PURPLE_D; h = 28; fs = 9;  }
    else if (isMonth) { bg = C.INDIGO_D; h = 22; fs = 8;  }
    else if (isNum)   { bg = C.INDIGO;   h = 18; fs = 7;  }
  ...
  for (let r = firstData; r <= lr; r++) {
    ...
    let metaBg = C.WHITE, metaFg = C.TEXT, metaWeight = 'normal';
    let activeColor = C.INDIGO, inactiveColor = C.WHITE, metaFontSize = 10;

    if (/^PT\d+$/i.test(code)) {
      metaBg = C.GANTT_PT_BG; metaFg = C.WHITE; metaWeight = 'bold';
      activeColor = C.GANTT_PT_ON; inactiveColor = C.GANTT_PT_OFF;
      sheet.setRowHeight(r, 26);
    } else if (/^A\d+(\.\d+)*$/i.test(code)) {
      metaBg = C.GANTT_TASK_BG; metaFg = C.WHITE; metaWeight = 'bold';
      activeColor = C.GANTT_TASK_ON; inactiveColor = C.GANTT_TASK_OFF;
      sheet.setRowHeight(r, 23);
    } else if (/^E\d+(\.\d+)*$/i.test(code)) {
      metaBg = C.GANTT_DEL_BG; metaFg = C.GANTT_DEL_FG; metaWeight = 'normal';
      activeColor = C.GANTT_DEL_ON; inactiveColor = C.GANTT_DEL_OFF;
      metaFontSize = 9;
      sheet.setRowHeight(r, 22);
    } else {
      sheet.setRowHeight(r, 22);
    ...
    sheet.getRange(r, 1, 1, Math.min(META, lc))
      ...
      .setFontWeight(metaWeight).setFontSize(metaFontSize);
    ...
    if (lc >= TL) {
      for (let c = TL; c <= lc; c++) {
        ...
        cell.setBackground(active ? activeColor : inactiveColor)
            .setFontColor(active ? activeColor : inactiveColor)
            .setHorizontalAlignment('center')
            .setFontWeight(active ? 'bold' : 'normal')
            .setFontSize(6);
      }
    }
    bottomBorder(row);
  ...
  for (let c = TL; c <= lc; c++) sheet.setColumnWidth(c, 24);
...
function formatSensibilidad(sheet) {
  ...
  for (let r = 1; r <= lr; r++) {
    ...
    if (a === '' && (bN.indexOf('plantilla ingeniero') !== -1 || bN.indexOf('plantilla otro') !== -1)) {
      if (lc >= 2) hdr(sheet.getRange(r, 2, 1, Math.min(3, lc - 1)), C.INDIGO_D);
      sheet.setRowHeight(r, 28);
      continue;
    }

    if (hasAny(a, ['categoria profesional', 'tabla salarial', 'nivel salarial', 'baja desproporcionada', 'baja desproporcionada temeraria anormal'])) {
      hdr(row, C.PURPLE_D);
      sheet.getRange(r, 1).setHorizontalAlignment('left');
      sheet.setRowHeight(r, 28);
      continue;
    ...
    if (hasAny(a, ['plantilla ingeniero superior', 'plantilla ingeniero', 'plantilla otro perfil', 'subcontratado'])) {
      ...
      for (let c = 2; c <= lc; c++) {
        const cell = sheet.getRange(r, c);
        if (typeof cell.getValue() === 'number') applyFmt(cell, a);
      }
      bottomBorder(row);
      continue;
    }
